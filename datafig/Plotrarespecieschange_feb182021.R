...
coversummaries = unique(cover[, .(site_code, year,  site_name,  plot,  year_trt , trt, totplotcover.yr.live, #  cover_nat_dom, cover_nat_sub,cover_tot_non.rare,
                                  sr_nat_sub, sr_non.nat_sub,  sr_INT, sr_NAT, sr_domspp, sr_rarespp, sr_subordspp, sr_non_rare_spp, 
                                  ...
                                  # sr_rarespp2, 
                                  #rare_spp.DI2,  
                                  #sr_non.Nfixer, N_fixer_cover.yr, sr_Nfixer, 
                                 # NonNative_cover.yr , Native_cover.yr ,  sr_annual, sr_peren, sr_null.lspan, sr_biennial ,
                                 # sr_indeter, AnnualPercentcover.yr, PerenPercentcover.yr , sr_graminoid, sr_forbs, sr_woody, LegumePercentcover.yr,
...
hist(mech.data$change_nonrare.nonnative, main = "Change in non-native, non-rare species richness", xlab = "Change in number of species per plot from year to year ",
     ylim = c(0, 800))
...
hist(mech.data$change_rare.nonnative,  main = "Change in non-native, rare species richness", xlab = "Change in number of species per plot from year to year ", 
     ylim = c(0, 800))
...
## plot change in non-rare species by site 
FigSX <- ggplot(data = mech.data, aes(x = change_nonrare.native)) + geom_histogram()+ facet_wrap(~site) + theme_bw() +
  geom_vline(xintercept=c(0,0), color = "blue", linetype="dashed") +
  labs(x = "Plot-level change in rare native species richness year to year") +  theme_bw() +
  theme(axis.title.y= element_text(size=14)) + theme(axis.title.x= element_text(size=12)) +
  theme(axis.text.y = element_text(size = 14)) + 
