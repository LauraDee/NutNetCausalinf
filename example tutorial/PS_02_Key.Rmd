...
This data has already been cleaned and processed, and is ready to use. It corresponds to what we use in the paper.
...
A first place to start is looking correlations within a single year (i.e., using cross-sectional variation). we use all data and simple look at the correlation between log live mass and log richness in two years, 2012 and 2013. Throughout, we are going to cluster the standard errors by plot to acknowledge correlation in errors terms within a plot across years (though when we use only a single year of data, this is equivalent to using heteroskedasticity-robust errors).
...
etable(SimpleCorr2012, SimpleCorr2013, 
          cluster = "newplotid", 
          drop = "Intercept", 
          subtitles = c("Data in 2012", "Data in 2013"))  
```

In the results above, observe the results switches signs and significance between the two years. Is that whack?! Sure is, non-experimental cross-sectional data often has problems with omitted variable bias, as this simple comparison shows.




## STUFF FROM TUTORIAL BELOW
