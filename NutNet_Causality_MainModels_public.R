...
bind_rows(
  coefs_ModPFE %>% mutate(reg = "Model 1"),
  coefs_ModPFE.2 %>% mutate(reg = "Model 2"),
  #coefs_ModPFE.3 %>% mutate(reg = "Model 3"),
  coefs_ModLD  %>% mutate(reg = "LD Model 1"),
  coefs_ModLD.2  %>% mutate(reg = "LD Model 2"),
  coefs_modiv1 %>% mutate(reg = "IV 1"),
  # coefs_modiv2  %>% mutate(reg = "IV 2: treated neighbor rich"),
  coefs_modiv3  %>% mutate(reg = "IV with both IVs")
) %>%
  ggplot(aes(x=reg, y=estimate, ymin=conf.low, ymax=conf.high, colour = reg)) +
  geom_pointrange() + theme_classic() +
  labs(Title = "Marginal effect of richness on live mass") +
  ...
  labs(
    title = "Effect size of Log Species Richness on Log Productivitiy",
    caption = "", x = "Model", y = "Estimate for log(richness) effect size") 
  ) +   theme(plot.title = element_text(hjust = 0.5)) + 
  theme(plot.title = element_text(face="bold", size = 18))
...
#################################################
## For Storing results to plot #####################
####################################################
all.results = data.frame()
# Estimate some model and call the clustered standard error function
# Say results are in a variable called results.

#The way to do it is: store coef and se info for each model
#coef.info = results[rownames(results)=="your.variable.of.interest.name",c(1,2)]
# Combine into the data frame
# all.results = rbind(all.results, list("Model"=1, "Est"=coef.info["Estimate"], "SE"=coef.info["Std. Error"]))
# Repeat the above two lines for each model, changing the model number each time. 
# You can also make the model a name instead of a number.
# You'll get a 3 column data frame with a model number (or name), estimate, and SE.

## store results for the SI
all.results.SI = data.frame()

#and for IV
all.results.IV = data.frame()
