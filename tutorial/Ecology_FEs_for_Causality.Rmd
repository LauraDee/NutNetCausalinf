...
A first place to start is looking at simple correlations. We will examine both the overall correlation and correlations within a single year (i.e., using cross-sectional variation). We proceed in a linear regression framework, use log of productibity (measured as live aboveground biomass) as the outcome and log species richness as the explanatory variable. We've found that the log-log specification captures the nature of the relationship in our data well (see the paper for details), also following prior studies (REFs). That is, we estimate and report $\beta$ in:
$$
...
### Common Ecological Design - Multivariate lm 
 ...
 Selection on observables -- and DAG for it.
...
Some confounding variables may be straightforward to measure, like xxxxx, others are not ........ LAURA TO FINISH
